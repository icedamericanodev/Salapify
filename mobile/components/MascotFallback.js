// MascotFallback: the pure react-native-Animated version of Pan, kept as the
// safety net behind the Skia version. If Skia ever fails to render (an API
// slip, a missing native module in some build), the error boundary in
// Mascot.js swaps to this, so the app never white-screens over the mascot.
//
// Pan built as independent layers that move on staggered timelines so the
// character feels like it has weight and momentum instead of sliding around
// as one flat block. Drawn entirely from plain views and geometry.
//
...
export default function MascotFallback({ size = 110, state = 'idle', style }) {
