...
import 'package:salapify/core/data/encrypted_store_coordinator.dart';
...
void main() {
  test(
    'steady state reads the encrypted store (not the fallback) and retires the redundant plaintext',
    () async {
      ...
      expect(
        ...
        _a,
        reason: 'the answer comes from encrypted',
      );
  ...
  test(
    'the migration run keeps the plaintext fallback, retiring it only on a later relaunch',
    () async {
      ...
      expect(
        fb.ledger,
        _a,
        reason: 'safety copy kept until a confirmed relaunch',
      );
      ...
      expect(
        fb.ledger,
        isNull,
        reason: 'retired after encryption survived a restart',
      );
    },
  );

  test(
    'an unopenable encrypted store never retires the plaintext fallback',
    () async {
      ...
      expect(
        fb.ledger,
        _a,
        reason: 'the fallback is exactly what we might still need',
      );
    },
  );

  test(
    'first run migrates the plaintext into the encrypted store, validated',
    () async {
      ...
      expect(
        fb.ledger,
        _a,
        reason: 'the plaintext fallback is left intact for now',
      );
      expect(repo.migratedThisRun, isTrue);
      expect(repo.lastSource, StorageEngine.encrypted);
      expect(repo.health.encrypted, isTrue);
  ...
  test('nothing anywhere reads as empty', () async {
    final repo = EncryptedStoreCoordinator(
      encrypted: MemRepo(),
      fallback: MemRepo(),
    );
  ...
  test(
    'a migration whose write fails serves the plaintext and does not claim encrypted',
    () async {
      ...
      expect(
        await repo.readLedger(),
        _a,
        reason: 'the plaintext still answers',
      );
  ...
  test(
    'an encrypted store that cannot open serves the plaintext and attempts no write',
    () async {
      final enc = MemRepo()..failReads = true;
      final fb = MemRepo()..ledger = _a;
      final repo = EncryptedStoreCoordinator(encrypted: enc, fallback: fb);
      ...
      expect(
        enc.writes,
        0,
        reason: 'no write is attempted against a store that could not open',
      );
    },
  );

  test(
    'every write goes to the encrypted store only, never the frozen fallback',
    () async {
      final enc = MemRepo()..ledger = _a;
      final fb = MemRepo()..ledger = _a;
      final repo = EncryptedStoreCoordinator(encrypted: enc, fallback: fb);
  ...
  test(
    'a write that the encrypted store rejects throws and is not durable',
    () async {
      final enc = MemRepo()
        ..ledger = _a
        ..failWrites = true;
  ...
  test(
    'SAFETY regression: an encrypted clear that fails does not resurrect the plaintext (order matters)',
    () async {
      ...
      expect(
        fb.ledger,
        isNull,
        reason: 'the resurrection source was cleared first',
      );
  ...
  test(
    'the undo snapshot is scoped to the encrypted era, never the frozen fallback',
    () async {
  ...
  test(
    'SAFETY regression: after erase, an encrypted-open failure does not resurrect from the fallback',
    () async {
      final enc = MemRepo()..ledger = _a;
      final fb = MemRepo()..ledger = _a;
      final repo = EncryptedStoreCoordinator(encrypted: enc, fallback: fb);
