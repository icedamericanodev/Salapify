...
import 'package:salapify/core/money/goals_calc.dart';

void main() {
  final g =
      jsonDecode(File('test/goldens/goals_goldens.json').readAsStringSync())
          as Map<String, dynamic>;

  test('goalNum parses money the same as the RN toNum', () {
    for (final c in (g['goalNum'] as List).cast<Map<String, dynamic>>()) {
      expect(
        goalNum(c['in'] as String),
  ...
  test('goalPercent matches the RN percent display', () {
    for (final c in (g['goalPercent'] as List).cast<Map<String, dynamic>>()) {
      expect(
        goalPercent(
          (c['saved'] as num).toDouble(),
          (c['target'] as num).toDouble(),
        ),
        c['out'] as int,
