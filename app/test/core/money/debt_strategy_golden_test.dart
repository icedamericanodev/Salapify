...
void main() {
  group('the credit card minimum payment trap', () {
    ...
    test(
      'the schedule is THINNED after month 36, so a screen must say about',
      () {
    ...
    test('a small balance clears without thinning at all', () {
      final CreditCardPayoffResult small = calculateCreditCardPayoff(
  ...
  group('snowball against avalanche', () {
    ...
    test('snowball clears smallest balance first', () {
      ...
      expect(r.snowball.orderOfPayoff, <String>[
        'Small Card',
        'Gadget Plan',
        'Big Card',
      ]);
    ...
    test('avalanche clears the highest rate first', () {
      ...
      expect(r.avalanche.orderOfPayoff, <String>[
        'Big Card',
        'Small Card',
        'Gadget Plan',
      ]);
    ...
    test('every debt is named once in each payoff order', () {
      for (final StrategySimulationResult s in <StrategySimulationResult>[
        r.snowball,
        r.avalanche,
      ]) {
        expect(s.orderOfPayoff.length, 3);
        expect(s.orderOfPayoff.toSet().length, 3);
        expect(s.orderOfPayoff.toSet(), <String>{
          'Small Card',
          'Big Card',
          'Gadget Plan',
        });
    ...
    test('more extra money never makes either strategy slower', () {
      int months(double extra, bool avalanche) {
        final StrategyComparison c = simulateDebtStrategies(
          debts: debts,
          extraMonthlyBudget: extra,
        );
        return avalanche
