...
void main() {
  ...
  group('credit cards', () {
    // POSITIVE IS OWED, and this default used to be negative.
    //
    // Both the engine and this group were ported from the prototype, where a
    // credit account stores what is owed as a negative number. Salapify does
    // the reverse everywhere: `summarize` subtracts a plain sum of liability
    // balances from assets, `creditUtilization` reads 12,000 against a
    // 40,000 limit as 30 percent, and the sample card carries 4,200. So the
    // fixture below was the one shape a real card never has, which is how
    // three green tests defended a reminder that could not fire.
    ...
    test('a card carrying a balance reminds before it falls due', () {
    ...
    test('the sample card, exactly as the app ships it, reminds', () {
      // The fixture above is a card this test file made up. This one is the
      // record a person actually meets on a new phone, and it is the shape
      // that proves the sign: 4,200 owed, positive, from seed_data.dart.
      ...
      expect(
        r.fresh,
        isNotEmpty,
        reason: 'the app shipped a card its own reminder could never see',
      );
    });

    test('a card at zero is silent, and one in credit too', () {
      // Money you do not owe is not a payment due. A card cleared to zero,
      // or overpaid into credit, must not be nagged about every month.
      expect(run(accounts: <Account>[card(balance: 0)]).fresh, isEmpty);
      expect(run(accounts: <Account>[card(balance: -2500)]).fresh, isEmpty);
    });

    test('a card with no due date cannot remind', () {
