...
void main() {
  ...
  group('mandatory contributions', () {
    test('a 30,000 monthly salary', () {
      final EmployeeTaxCalculation r =
          calculateEmployeeTaxDeductions(inputSalary: 30000);
      expect(r.sss, 1350);
    ...
    test('SSS caps at a 30,000 salary credit, so it stops growing', () {
      final EmployeeTaxCalculation mid =
          calculateEmployeeTaxDeductions(inputSalary: 65000);
      final EmployeeTaxCalculation high =
          calculateEmployeeTaxDeductions(inputSalary: 700000);
      expect(mid.sss, 1350);
    ...
    test('PhilHealth caps at 2,500 and Pag-IBIG at 200', () {
      final EmployeeTaxCalculation r =
          calculateEmployeeTaxDeductions(inputSalary: 200000);
    ...
    test('a low salary is floored, not taken to zero', () {
      final EmployeeTaxCalculation r =
          calculateEmployeeTaxDeductions(inputSalary: 12000);
      expect(r.sss, 540);
  ...
  group('withholding tax across the brackets', () {
    void check(
      ...
      test(label, () {
        final EmployeeTaxCalculation r =
            calculateEmployeeTaxDeductions(inputSalary: salary);
    ...
    check('25 percent band, 200,000', 200000, 195950, 42326.668999999994, 153623);
  ...
  group('pay frequency', () {
    test('semi-monthly, bi-weekly and annual land on the same monthly salary',
        () {
    ...
    test('an annual figure divides down to the same monthly result', () {
      ...
      final EmployeeTaxCalculation monthly =
          calculateEmployeeTaxDeductions(inputSalary: 65000);
  ...
  group('allowances, overtime and night differential', () {
    test('taxable extras raise the tax, non-taxable ones do not', () {
      ...
      expect(r.grossMonthlyIncome - r.taxableIncome - r.totalContributions,
          closeTo(2000, eps));
    });

    test('a zero salary returns a zeroed result rather than dividing by it',
        () {
      final EmployeeTaxCalculation r =
          calculateEmployeeTaxDeductions(inputSalary: 0);
  ...
  group('13th month', () {
    test('the first 90,000 is exempt and the excess is taxed', () {
      final EmployeeTaxCalculation low =
          calculateEmployeeTaxDeductions(inputSalary: 65000);
    ...
    test('months worked is clamped to 0 and 12 at both ends', () {
      expect(
        calculate13thMonthPay(basicMonthlySalary: 30000, monthsWorked: 0)
            .calculatedGrossAmount,
        0,
      );
      expect(
        calculate13thMonthPay(basicMonthlySalary: 30000, monthsWorked: 18)
            .calculatedGrossAmount,
        30000,
      );
    });
    ...
    test('above the ceiling, only the excess is withheld', () {
      final ThirteenthMonthPlan plan =
          calculate13thMonthPay(basicMonthlySalary: 120000);
    ...
    test('the allocation blueprint splits the net into five and sums to it',
        () {
      final ThirteenthMonthPlan plan =
          calculate13thMonthPay(basicMonthlySalary: 120000);
      expect(plan.allocations.length, 5);
      expect(
        plan.allocations.fold<int>(0, (int s, ThirteenthMonthAllocation a) => s + a.percentage),
        100,
      );
      final double total = plan.allocations
          .fold<double>(0, (double s, ThirteenthMonthAllocation a) => s + a.targetAmount);
  ...
  group('freelance, 8 percent against graduated', () {
    test('the 8 percent route deducts 250,000 first', () {
      final FreelanceTaxCalculation r =
          calculateFreelanceTax(annualGrossIncome: 1200000);
    ...
    test('at 250,000 the 8 percent route owes nothing at all', () {
      final FreelanceTaxCalculation r =
          calculateFreelanceTax(annualGrossIncome: 250000);
      expect(r.taxableBase, 0);
      expect(r.estimatedTaxDue, 0);
      expect(r.effectiveTaxRate, 0);
    });

    test('8 percent beats graduated at this income, which is the whole point',
        () {
      final double git =
          calculateFreelanceTax(annualGrossIncome: 1200000).estimatedTaxDue;
