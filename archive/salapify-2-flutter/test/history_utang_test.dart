...
import 'package:salapify/data/store.dart'
    show SalapifyStore, ensureUniqueTxnIds, storageKey;
...
void main() {
  test(
    'removeEntry reverses the balance and undo restores it exactly',
    () async {
      ...
      double cash() =>
          ((store.data['accounts'] as List)
                      ...
                      .firstWhere((a) => a['id'] == 'cash')['balance']
                  as num)
              .toDouble();
      expect(cash(), 1250);
  ...
  test('only plain income and expense rows with a real id are deletable', () {
    ...
    expect(
      isDeletable({'id': 'a', 'type': 'income', 'source': 'receivable'}),
      isFalse,
    );
    ...
    expect(
      isDeletable({'id': 'paid1', 'type': 'expense'}, lockedIds: {'paid1'}),
      isFalse,
    );
  });
  ...
  test('the store boundary assigns missing transaction ids and splits '
    ...
    final clean = ensureUniqueTxnIds(
      sanitizeData({
        'transactions': [
  ...
  test(
    'deleting one of two same-id entries can no longer swallow money',
    () async {
