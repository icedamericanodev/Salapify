...
void main() {
  ...
  test(
    'import through the durable engine persists, mirrors to the shadow file, and reloads',
    () async {
  ...
  test(
    'a plain revert to the old store reads the exact same up-to-date data',
    () async {
      SharedPreferences.setMockInitialValues({});
      final store = SalapifyStore(repository: durable());
      await store.load();
      ...
      final reverted = SalapifyStore(
        repository: const SharedPrefsLedgerRepository(),
      );
      await reverted.load();
  ...
  test(
    'an existing ledger is copied into the shadow file on first durable load',
    () async {
