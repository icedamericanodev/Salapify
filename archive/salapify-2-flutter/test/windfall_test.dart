...
Map<String, dynamic> baseData() => {
  ...
  'transactions': [_exp('2026-05-20', 10000), _exp('2026-06-20', 10000)],
};
...
void main() {
  group('splitWindfall waterfall', () {
    ...
    test(
      'a big windfall clears BNPL, fills the fuller fund, the goal, leftover',
      () {
    ...
    test(
      'a 0% BNPL is offered as a clear-it slice, not flagged as rate-less',
      () {
        final data = baseData();
        data['debts'] = [
          {
    ...
    test(
      'a rate-less card or loan IS flagged, since it should have interest',
      () {
        final data = baseData();
        data['debts'] = [
          {
  ...
  group('honest degradation and guards', () {
    test(
      'with no spending history, a 10k floor cushion is used and flagged',
      () {
        final data = baseData();
        data['accounts'] = [
    ...
    test(
      'a set-aside larger than the windfall is clamped, nothing to split',
      () {
    ...
    test('zero and junk amounts are not applicable', () {
      ...
      expect(
        splitWindfall(baseData(), ref, amount: 'wala')['applicable'],
        false,
      );
    });
