...
void main() {
  ...
  group('newPaluwagan normalizes a raw form to safe defaults', () {
    ...
    test(
      ...
      () => expect(p['cadence'], 'monthly'),
    );
    test('reads a string amount the JS way', () => expect(p['amount'], 2500));
    test(
      'blank name falls back to Paluwagan',
      () => expect(p['name'], 'Paluwagan'),
    );
    test(
      'startDate defaults to the reference day',
  ...
  group('paluwaganStatus computes the decision figures', () {
    ...
    test(
      'three cycles have come due by 15 July',
    ...
    test(
      'a full round is zero sum: total in equals payout',
  ...
  group('the monthly commitment feeds safe-to-spend', () {
    test(
      'a monthly cadence commits the amount',
    ...
    test('weekly and kinsenas normalize to per-month', () {
      ...
      final wk = paluwaganMonthlyCommitment({
        ...base,
        'cadence': 'weekly',
        'amount': 500,
      }, ref);
      expect(wk, closeTo(500 * (52 / 12), 1e-6));
      final kn = paluwaganMonthlyCommitment({
        ...base,
        'cadence': 'kinsenas',
        'amount': 500,
      }, ref);
  ...
  group('early-recipient net must be paired with the remaining owed', () {
    ...
    test(
      'the remaining obligation is real and non-zero',
  ...
  group('a degenerate one-person group is refused', () {
    test(
      'paluwaganStatus floors members at two',
      () => expect(
        paluwaganStatus({...monthly, 'members': 1}, ref)['members'],
        2,
      ),
    );
  });
