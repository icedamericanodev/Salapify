...
void main() {
  group('the real course progresses correctly', () {
    ...
    test(
      ...
      () async {
        ...
        await store.markExpansionLessonStarted(_pathId, sbVerifyBeforeYouInvest);
        expect(
          store.expansionProgressFor(_pathId)[sbVerifyBeforeYouInvest],
          LessonState.completed,
          reason: 'starting again must never demote a completed lesson',
        );
        await store.markExpansionLessonApplied(_pathId, sbVerifyBeforeYouInvest);
        expect(
          store.expansionProgressFor(_pathId)[sbVerifyBeforeYouInvest],
          LessonState.applied,
  ...
  group('isolation from the core 22 and from the pilot', () {
    test(
      'progressing every lesson in this course leaves core lessonProgress '
      'empty',
      () async {
        final store = await _freshStore();
        for (final id in _lessonIds) {
    ...
    test('completing this course never touches the pilot\'s own progress', () async {
      final store = await _freshStore();
      for (final id in _lessonIds) {
        await store.markExpansionLessonCompleted(_pathId, id);
      }
  ...
  group('persistence across a simulated restart', () {
    test(
      'real course progress survives a fresh store reading the same disk '
      'state',
      () async {
        final store = await _freshStore();
        await store.markExpansionLessonCompleted(_pathId, sbOwnerOrLender);
  ...
  group('backup export and restore', () {
    test('real course progress round-trips through export and import', () async {
      final store = await _freshStore();
      await store.markExpansionLessonCompleted(_pathId, sbOwnerOrLender);
    ...
    test(
      'a backup with progress in both the pilot and this course preserves '
      'both',
      () async {
        final store = await _freshStore();
    ...
    test(
      'a backup with no progress in this course omits it from '
      'expansionProgress',
      () async {
        final store = await _freshStore();
        final text = store.exportBackupText();
