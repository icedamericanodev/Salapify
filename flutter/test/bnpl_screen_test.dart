...
void main() {
  ...
  testWidgets('a real 0% reassures and a fee unmasks it', (tester) async {
    ...
    await tester.enterText(
      find.widgetWithText(TextField, 'e.g. 12,000'),
      '12,000',
    );
    await tester.enterText(find.widgetWithText(TextField, 'e.g. 6'), '6');
    await tester.enterText(
      find.widgetWithText(TextField, 'e.g. 2,100'),
      '2000',
    );
    await tester.pumpAndSettle();

    await tester.scrollUntilVisible(
      find.textContaining('costs the same as paying cash today'),
      200,
    ...
    await tester.scrollUntilVisible(
      find.text('UPFRONT FEE (OPTIONAL)'),
      -200,
      scrollable: find.byType(Scrollable).first,
    );
    await tester.pumpAndSettle();
    ...
    await tester.pumpAndSettle();
    await tester.scrollUntilVisible(
      find.textContaining('more than paying cash'),
      200,
      scrollable: find.byType(Scrollable).first,
    );
    await tester.pumpAndSettle();
    expect(find.textContaining('₱480 more than paying cash'), findsOneWidget);
  ...
  testWidgets('underpaying numbers get the honest check state', (tester) async {
    final store = SalapifyStore();
    await tester.pumpWidget(SalapifyApp(store: store));
    await openBnpl(tester);
    ...
    await tester.pumpAndSettle();
    expect(
      find.textContaining('which is less than the ₱12,000 cash price'),
      findsOneWidget,
    );
